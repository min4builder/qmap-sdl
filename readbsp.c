/*
 * QMAP: Quake level viewer readbsp.c This file is a compilation of several
 * files released by John Carmack as part of the Quake utilities source
 * release.  No redistribution limitations appeared in that distribution.  I
 * don't claim any copyright either. 
 */
...
SwapBSPFile(qboolean todisk)
{
	...
	for (i = 0; i < numvertexes; i++) {
		for (j = 0; j < 3; j++)
			dvertexes[i].point[j] =
				LittleFloat(dvertexes[i].point[j]);
	}
	...
	for (i = 0; i < numplanes; i++) {
		for (j = 0; j < 3; j++)
			dplanes[i].normal[j] =
				LittleFloat(dplanes[i].normal[j]);
	...
	for (i = 0; i < numtexinfo; i++) {
		for (j = 0; j < 8; j++)
			texinfo[i].vecs[0][j] =
				LittleFloat(texinfo[i].vecs[0][j]);
	...
	for (i = 0; i < numleafs; i++) {
		...
		dleafs[i].firstmarksurface =
			LittleShort(dleafs[i].firstmarksurface);
		dleafs[i].nummarksurfaces =
			LittleShort(dleafs[i].nummarksurfaces);
	...
	for (i = 0; i < numclipnodes; i++) {
		...
		dclipnodes[i].children[0] =
			LittleShort(dclipnodes[i].children[0]);
		dclipnodes[i].children[1] =
			LittleShort(dclipnodes[i].children[1]);
	}
...
LoadBSPFile(char *filename)
{
	...
	numclipnodes =
		CopyLump(LUMP_CLIPNODES, dclipnodes, sizeof(dclipnode_t));
	numfaces = CopyLump(LUMP_FACES, dfaces, sizeof(dface_t));
	nummarksurfaces =
		CopyLump(LUMP_MARKSURFACES, dmarksurfaces,
