...
void
free_next_surface(void)
{
	if (surface_head == surface_tail)
		return;
...
int
allocate_cached_surface(int size)
{
	int surf;
...
get_tmap(bitmap *bm, int face, int tex, int ml, float *u, float *v)
{
	...
	*v = surface[surf].v = v0;
	...
	y = v0 % raw.ht;
	if (y < 0)
		y += raw.ht;	// fixup for signed mod
	x0 = u0 % raw.wid;
	if (x0 < 0)
		x0 += raw.wid;	// fixup for signed mod
...
build_block(char *out, bitmap *raw, int x, int y)
{
	...
	fix c, dc;
	int a, b, h, c0, c1, c2, c3, step = global_step, row =
		global_row - step;
