...
void
qmap_draw_affine(int n, char *dest, fix u, fix v, fix du, fix dv)
{
	if (is_cached) {
		while (n--) {
			int iu = fix_int(u);
...
void
qmap_draw_span(int y, int sx, int ex)
{
	...
	for (;;) {
		...
		if (len == 1) {
			...
			qmap_draw_affine(len, qmap_buf + qmap_row_table[y] + sx,
				u, v, 0, 0);
			return;
		}
		...
		qmap_draw_affine(len, qmap_buf + qmap_row_table[y] + sx, u, v,
			du, dv);
		if (last)
			break;
...
void
qmap_set_texture_gradients(double *tmap_data)
{
	int i;
	for (i = 0; i < 9; ++i)
